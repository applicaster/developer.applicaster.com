...
| `source`              | URL for the source video file.                                                                                                                                                                                                                                                   | Yes (unless using `alternative_streams`) | **only** video         |
| `alternative_streams` | Streaming URLs for this item. See below for further explanation. **Note: Only use this tag if you are sure the provided URLs are HLS streams that comply to Apple’s AppStore submission guidelines. using non compliant streams can result in the app being rejected by Apple.** | No                                       | **only** video         |
...
| `link_url`            | an HTTP(S) URL that hosts the web content that will be displayed in the webview                                                                                                                                                                                                  | Yes (for the `link` type)                | **only** link          |
| `orientation`         | Supported device orientation. One of `portrait`, `landscape`, or `both`. default: `both`                                                                                                                                                                                         | No                                       | **only** link          |
| `show_navigation`     | Should the application show back/forward buttons. Either `true` or `false`. default: `true`                                                                                                                                                                                      | No                                       | **only** link          |
| `show_close_button`   | Should the application show a close button. Either `true` or `false`. default: `true`                                                                                                                                                                                            | No                                       | **only** link          |
| `enable_zoom`         | Can the user change scale of the webview. Either `true` or `false`. default: `false`                                                                                                                                                                                             | No                                       | **only** link          |
| `date_published`      | The date the item was published in [ISO8601 combined date and time format](http://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations)
| No                                       | all                    |
...
| `rtsp`    | An RTSP stream URL. This will be used only for old devices that have no HLS support | No       |
