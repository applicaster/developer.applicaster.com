...
In this step, we will prepare your plugin workspace. It will have a structure like this:
...
- create a folder and initialize your git repository by running `git init`
- run `npm init` to create your package.json file for your plugin repository. add the following properties to enable yarn workspaces
  - `"private": true`
  - `workspaces: ["plugin", "development-app"]`
- create a folder named `plugin`. In this folder, create another npm package by running `npm init`. the name in the package.json here will be the actual name of your plugin package. It should be scoped with `@applicaster` domain, and be prefixed with `quick-brick` like this `@applicaster/quick-brick-my-plugin-name`. Make sure the `main` property of this package.json points to the entry point of your plugin
- :warning: Take care, the babel transpiler _will fail miserably in builds_ without `@applicaster/quick-brick-` scope and prefix, it depends on both for picking the dependency correctly
...
- create a `metro.config.js` file at the root of the workspace. Since we're not running a react-native bundle whose entry point is at the root of the workspace, this is required to provide the relevant configuration to the react-native metro bundler :
...
- create a `.gitignore` and a `.npmignore` file at the root of the workspace, and add this inside
...
#### 2. Create your plugin manifests
...
#### 2.b Declaring native dependencies to your plugin manifests
...
2. add the references to add to the gradle file
   2.1. if the code is non-compiled code, you need to add it as a `project_dependencies`
   2.2. if the code is a pre-compiled aar file, you need to add it as a `extra_dependencies`
3. add a reference to a class that can be instantiated during the plugin initialisation. It can be the class name of the react package if your plugin doesn't have its own native implementation
...
#### 3. Set up your app in Zapp

Now that your manifests have been uploaded, you can add your plugin to your app, where you want to use it. Refer to the Zapp & UI builder documentation to know how to add your plugin dependending on its type.

#### 4. Set up your local QuickBrick app
...
- If you want to run your app on tvos, clone [Zapp-tvos](https://github.com/applicaster/Zapp-tvos) and follow the instruction to set up your app. Open Xcode, and start your app in the simulator.
