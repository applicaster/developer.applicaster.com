...
The `free` key can be defined on the extensions object to indicate whether an item is free or not. The value can be a String representation of a Boolean (true/false) or a Boolean. If the extension key is not defined or a bad value was supplied the default item behavior will be as free.
